...
export default function MissionsScreen({ team, onSelectMission, onLogout }: Props) {
  ...
  useEffect(() => {
    fetch('/api/settings')
      .then(r => r.json())
      .then(d => setVisibleIds(d.visible_missions ?? MISSIONS.map(m => m.id)))
      .catch(() => setVisibleIds(MISSIONS.map(m => m.id)));
  }, []);

  const visibleMissions = visibleIds === null
    ? []
    : MISSIONS.filter(m => visibleIds.includes(m.id));
    ...
    <>
      ...
      <div className="container fade-in">
        ...
        {visibleIds === null ? (
          ...
          <div className="empty-state">No missions are available yet. Check back soon!</div>
        ) : (
